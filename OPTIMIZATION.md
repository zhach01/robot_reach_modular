...
(`lib/dynamics/DynamicsHTM_torch`, `lib/kinematics/HTM_kinematics_torch`,
`lib/movements/HTM_torch`) — there is no separate `_OPTIMIZED` copy anymore; the
base modules *are* the optimized implementation, shared by `skeleton_torch` and
...
## 2. Analytic 2-DOF fast-path (`skeleton_torch.TwoDofArm`)
...
`energy_tank_controller_torch`).

## 3. NumPy simulation backbone

The live per-timestep numpy path (`environment_numpy`, `effector_numpy`,
`skeleton_numpy`, `sim/simulator_numpy`) was profiled and tightened — all
bit-identical:
...
- `muscles_numpy._integrate`: attribute localization + shared branch-mask hoist.
- `muscle_tools_numpy`: memoize muscle channel indices (saves ~66 `list.index`/step in the bisection).
