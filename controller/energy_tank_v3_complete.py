...
# NOTE: this module intentionally exports only its own `EnergyTankV3Params` /
# `EnergyTankV3Controller` names. It used to also alias the generic
# `EnergyTankParams` / `EnergyTankController`, which collided with the canonical
# implementation in controller/energy_tank_controller.py (the one the PASSIVITY
# scripts import). The aliases were removed to end that name collision (audit
# coherence). Use controller.energy_tank_controller for the canonical tank
# controller, or import the V3 classes by their explicit names from here.
