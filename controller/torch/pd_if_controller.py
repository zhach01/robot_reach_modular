...
- Uses HTM-based kinematics/dynamics:
    * lib.kinematics.torch.HTM_kinematics.{geometricJacobian, geometricJacobianDerivative}
    * lib.dynamics.torch.DynamicsHTM.{inertiaMatrixCOM, centrifugalCoriolisCOM, gravitationalCOM}
- Uses Torch guard/muscle utilities:
    * utils.torch.kinematics_guard
    * utils.torch.dynamics_guard
    * utils.torch.muscle_guard
    * utils.torch.math_utils
    * utils.torch.linear_utils
    * utils.torch.telemetry
    * muscles.torch.muscle_tools
...
from utils.torch.linear_utils import nnls_small_active_set  # parity import (unused)
...
import lib.kinematics.torch.HTM_kinematics as _kin
import lib.dynamics.torch.DynamicsHTM as _dyn
...
class PDIFController:
    ...
    def _compute_dynamics_(self, q: Tensor, qd: Tensor):
        ...
        if p.enable_gravity_comp:
            g_any = self.env.skeleton.gravity_vector(q)  # (n,1) or (B,n,1)
            if g_any.dim() == 3:
                g = g_any[0].view(-1)
            else:
                g = g_any.view(-1)
    ...
    def compute_(self, x_d: Tensor, xd_d: Tensor, xdd_d: Tensor) -> Dict[str, Any]:
        ...
        # ------------------------------------------------------------------
        J_any = self.env.skeleton.geometric_jacobian(q)  # (6,n) or (B,6,n)
        if J_any.dim() == 3:
            J = J_any[0]
        else:
            J = J_any
