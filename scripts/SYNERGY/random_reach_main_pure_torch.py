#!/usr/bin/env python3
# scripts/SYNERGY/random_reach_main_pure_torch.py
# Torch counterpart of random_reach_main_pure.py -- the PURE K-synergy baseline
# (RESEARCH BASELINE, not canonical). Loads the same NMF W model.
...
W_MODEL = "synergy/saved_model/W_model.npz"
...
def build_env_torch(pc: PlantConfig):
    ...
    env = EnvironmentTorch(
        ...
        name="RandomReachSynergyPureEnvTorch",
    )
...
def main():
    print("[RandomReach Synergy-Pure Torch] demo starting ...")
    ...
    ctrl = SynergyPureController(env, arm, SynergyPureParams(c_max=6.0, nnls_iters=80))
    if os.path.exists(W_MODEL):
        ctrl.set_W(torch.tensor(np.load(W_MODEL)["W"], dtype=torch.get_default_dtype(), device=device))
        print(f"[pure synergy] loaded W from: {W_MODEL}  (shape={tuple(ctrl.get_W().shape)})")
    else:
        print(f"[pure synergy] WARNING: no W model at {W_MODEL}; using random default (won't track).")
    ...
    try:
        ...
        print(f"[RandomReach Synergy-Pure Torch] WARNING: plotting failed: {e}")
    print("[RandomReach Synergy-Pure Torch] demo complete.")
    plt.show()
