#!/usr/bin/env python3
# scripts/MPC/main_random_reach_bc_a_torch.py
# Torch counterpart of main_random_reach_bc_a.py (canonical Hybrid/BC = activation
# policy). Loads the SAME BC checkpoint (models/random_reach_bc_a.pt), which is a
# torch MLP trained by behavior-cloning the (improved) NMPC teacher.
...
from controller.torch.hybrid_bc_a import RLPolicy, RLPolicyParams, RLControllerATorch, RLControllerAParams
...
CKPT = "models/random_reach_bc_a.pt"
...
def main():
    print("[RandomReach BC-a Torch] demo starting ...")
    pc = PlantConfig(); tc = TrajectoryConfig(); run = RunConfig()

    if not os.path.exists(CKPT):
        print(f"[BC-a] no checkpoint at '{CKPT}'. Train first:")
        print("  1) python -m scripts.MPC.make_data.make_teacher_dataset_random_reach_a --episodes 32 --out data/random_reach_a_ds.npz")
        print("  2) python -m scripts.MPC.train_rl.train_bc_random_reach_a --dataset data/random_reach_a_ds.npz --epochs 200 --out " + CKPT)
        return
    ...
    obs_dim, act_dim = 14 + 5 * m, m
    pi = RLPolicy(RLPolicyParams(obs_dim=obs_dim, act_dim=act_dim, hidden=(128, 128), device=str(device)))
    pi.load(CKPT)
    print(f"[BC-a] loaded policy: {CKPT}  (obs_dim={obs_dim}, act_dim={act_dim})")

    # center-out trajectory (4 targets out-and-back)
    ...
    try:
        ...
        print(f"[RandomReach BC-a Torch] WARNING: plotting failed: {e}")

    print("[RandomReach BC-a Torch] demo complete.")
    plt.show()
