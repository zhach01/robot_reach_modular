...
def main():
    ...
    lambda_surf = np.array([18.0,18.0])   # np.array([16.0, 16.0]) slope of sliding surface on position error (e_x)
    ...
    K_switch    = np.array([18.0, 18.0])    # amplitude of switching/robust term
    ...
    phi         = np.array([0.075, 0.075])    # boundary layer half-width for sat(s/phi)
    ...
    print("Center-out (Sliding-Mode) complete.")
    plt.show()
