#!/usr/bin/env python3
# main_random_reach_sliding_torch.py
# Torch counterpart of main_random_reach_sliding.py (sliding-mode PD/IF).
...
def build_env_torch(pc: PlantConfig):
    ...
    env = EnvironmentTorch(
        ...
        name="RandomReachSlidingEnvTorch",
    )
...
def main():
    print("[RandomReach Sliding Torch] demo starting ...")
    ...
    ctrl = SlidingModeController(env, arm, _sliding_params(num, toggles, ifc))
    ...
    try:
        ...
        print(f"[RandomReach Sliding Torch] WARNING: plotting failed: {e}")

    print("[RandomReach Sliding Torch] demo complete.")
    plt.show()
