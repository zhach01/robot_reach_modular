...
run short closed-loop simulations and take a couple of minutes total. The GPU
smoke test (`test_gpu_smoke.py`) auto-skips when no CUDA device is present.
...
| `test_controllers_reach.py` | H2/H3 (sliding-mode) + PD/IF + OSC tracking |
| `test_orphan_controllers_reach.py` | energy-tank analytic fast-path reach |
