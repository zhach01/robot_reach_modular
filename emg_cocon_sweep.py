...
# precompute reach inputs + EMG once
REACH=[]
for tk in D:
...
def run_cocon(coc):
    ...
    for sh,el,Ew in REACH:
        ...
        logs=TargetReachSimulator(env_,arm,pid(env_,arm,coc),T(xd,xv,xa,dt),ths.shape[1]).run();act=logs.act_log[:logs.k]
        tm=np.linspace(0,1,act.shape[0]);te=np.linspace(0,1,Ew.shape[1])
        for col,row,lab in PAIRS:
            mo=np.interp(ph,tm,act[:,col]);mo/=(mo.max() or 1);me=np.interp(ph,te,envf(Ew[row]));me/=(me.max() or 1)
            acc[lab][0].append(mo);acc[lab][1].append(me)
    out={}
    for _,_,lab in PAIRS:
        Mm=np.array(acc[lab][0]).mean(0);Em=np.array(acc[lab][1]).mean(0);out[lab]=np.corrcoef(Mm,Em)[0,1]
    return out
print("cocon  | ShoFlex ShoExt Biceps Triceps | mean")
for coc in [0.0,0.05,0.10,0.20]:
    r=run_cocon(coc); mn=np.mean(list(r.values()))
    print(f"{coc:.2f}   |  {r['ShoFlex']:+.2f}   {r['ShoExt']:+.2f}  {r['Biceps']:+.2f}  {r['Triceps']:+.2f}  | {mn:+.2f}")
