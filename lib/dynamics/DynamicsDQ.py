...
from sympy import *
from lib.dynamics.fastsymp import fast_simplify, tidy
...
def dqInertiaMatrixCOM(robot: object, symbolic=False):
    ...
    for j in range(len(robot.COMs)):
        ...
        R = (
            leftOperator(fkCOMDQ[j + 1][0:4, :], symbolic)
            * rightOperator(conjugateDQ(fkCOMDQ[j + 1][0:4, :], symbolic), symbolic)
            if symbolic
            else leftOperator(fkCOMDQ[j + 1][0:4, :]).dot(
                rightOperator(conjugateQ(fkCOMDQ[j + 1][0:4, :]))
            )
        )

        # Inertia with respect to center of mass: Icom = R^T * I * R
        Icom = (
            R.T * robot.symbolicQuaternionInertia[j] * R
            if symbolic
            else R.T.dot(robot.quaternionInertia[j]).dot(R)
        )
...
def dqPotentialEnergyCOM(
    robot: object, g=np.array([[0], [0], [0], [-9.80665]]), symbolic=False
):
...
def dqGravitationalCOM(
    ...
    for j in range(len(robot.COMs)):
        ...
        # m * (JdqCOM)^T * g
        G += (
            robot.symbolicMass[j] * JdqCOM[4:, :].T * g
            if symbolic
            else robot.mass[j] * (JdqCOM[4:, :].T).dot(g)
        )
