...
sys.path.append(sys.path[0].replace(r"/lib/kinematics/numpy", r""))

# Libraries
from lib.kinematics.numpy.HTM_kinematics import *
from lib.movements.numpy.HTM import crossMatrix
import numpy as np
from sympy import *
from lib.dynamics.numpy.fastsymp import tidy
...
def geometricCOMStateSpace(robot: object, COM: int, symbolic=False):
    ...
    # Calculate state-space equation (symbolic or numerical)
    Xd = tidy(Jcom * robot.qdSymbolic) if symbolic else Jcom.dot(robot.jointsVelocities)
...
def velocityPropagation(
    robot: object, v0: np.array, w0: np.array, qd: np.array, symbolic=False
):
    ...
    # Initial conditions
    V = [Matrix.vstack(v0, w0) if symbolic else np.append(v0, w0, axis=0)]
    ...
    for k in range(1, m):
        ...
        Mi = (
            Matrix([[eye(3), -ri], [zeros(3), eye(3)]])
            if symbolic
            else np.append(
                np.append(np.eye(3), -ri, axis=1),
        ...
        if any(element == True for element in containedJoints):
            ...
            vJoint = (
                Mi * Matrix.vstack(zeros(3, 1), z) * qd[joint]
                if symbolic
                else Mi.dot(np.append(np.zeros((3, 1)), z.reshape((3, 1)), axis=0))
                * qd[joint]
            )
            ...
            vJoint = zeros(6, 1) if symbolic else np.zeros((6, 1))

        # Calculate Velocities (both linear and angular) up to this point
        v = tidy((Mi * V[-1]) + vJoint) if symbolic else (Mi.dot(V[-1])) + vJoint

        # Append each calculated velocity
        V.append(v if symbolic else v)
...
def accelerationPropagation(
    ...
    # Initial conditions
    dV = [Matrix.vstack(dv0, dw0) if symbolic else np.append(dv0, dw0, axis=0)]
    ...
    for k in range(1, m):
        ...
        if any(element == True for element in containedJoints):
            ...
            M = (
                Matrix(
                    [
                        [zeros(3), ri * crossMatrix(z, symbolic)],
                        [zeros(3), -crossMatrix(z, symbolic)],
                    ]
                )
                if symbolic
                else np.append(
                    np.append(np.zeros((3, 3)), ri.dot(crossMatrix(z)), axis=1),
                    np.append(np.zeros((3, 3)), -crossMatrix(z), axis=1),
                    axis=0,
                )
            )
            ...
            dVjoint = (
                (M * V[k] * qd[joint])
                + (Mi * Matrix.vstack(zeros(3, 1), z) * qdd[joint])
                if symbolic
                else (M.dot(V[k]) * qd[joint])
                + (
                    Mi.dot(np.append(np.zeros((3, 1)), z.reshape((3, 1)), axis=0))
                    * qdd[joint]
                )
            )
        ...
        if symbolic:
            coriolis = Matrix.vstack(
                V[k][3:6, :].cross(V[k][3:6, :].cross(r)), zeros(3, 1)
            )
            dv = tidy((Mi * dV[-1]) + dVjoint + coriolis)
        else:
            coriolis = np.append(
                np.cross(V[k][3:6], np.cross(V[k][3:6], r, axis=0), axis=0),
                np.zeros((3, 1)),
                axis=0,
            )
            dv = Mi.dot(dV[-1]) + dVjoint + coriolis

        # Append each calculated acceleration
        dV.append(dv if symbolic else dv)
...
def velocityPropagationCOM(
    ...
    # Initial conditions
    Vcom = [
        Matrix.vstack(vCOM0, wCOM0) if symbolic else np.append(vCOM0, wCOM0, axis=0)
    ]
    ...
    for k in range(1, m):
        ...
        # If any joint is in the current reference frame
        if any(element == True for element in containedCOMs) and any(
            element == True for element in containedJoints
        ):
            ...
            vCOM = (
                tidy((Mi * V[joint]) + vJoint)
                if symbolic
                else Mi.dot(V[joint]) + vJoint
            )

            # Append each calculated velocity
            Vcom.append(vCOM if symbolic else vCOM)
        ...
        # If there's no joint in current frame
            ...
            # Calculate velocity up to this point
            vCOM = tidy(Mi * V[k - 1]) if symbolic else Mi.dot(V[k - 1])
...
def accelerationPropagationCOM(
    ...
    # Initial conditions
    dVcom = [
        Matrix.vstack(dvCOM0, dwCOM0) if symbolic else np.append(dvCOM0, dwCOM0, axis=0)
    ]
    ...
    for k in range(1, m):
        ...
        if any(element == True for element in containedCOMs) and any(
            ...
            if symbolic:
                coriolis = Matrix.vstack(
                    Vcom[COM][3:6, :].cross(Vcom[COM][3:6, :].cross(r)), zeros(3, 1)
                )
                dvCOM = tidy((Mi * dV[joint]) + dvJoint + coriolis)
            else:
                coriolis = np.append(
                    np.cross(
                        Vcom[COM][3:6], np.cross(Vcom[COM][3:6], r, axis=0), axis=0
                    ),
                    np.zeros((3, 1)),
                    axis=0,
                )
                dvCOM = Mi.dot(dV[joint]) + dvJoint + coriolis

            # Append each calculated velocity
            dVcom.append(dvCOM if symbolic else dvCOM)

        # If there's no joint in current frame
        elif any(element == True for element in containedCOMs):

            # Relative position between center of mass and i-th frame
            r = fkCOMHTM[COM][0:3, -1] - fkHTM[k - 1][0:3, -1]
            ...
            if symbolic:
                coriolis = Matrix.vstack(
                    Matrix(Vcom[COM][3:6, :]).cross(Matrix(Vcom[COM][3:6, :]).cross(r)),
                    zeros(3, 1),
                )
                dvCOM = tidy(Mi * dV[k - 1] + coriolis)
            else:
                coriolis = np.append(
                    np.cross(
                        Vcom[COM][3:6], np.cross(Vcom[COM][3:6], r, axis=0), axis=0
