...
def forwardDQ(robot: object, symbolic=False):
    ...
    fkDQ = (
        ...
        if symbolic
        else np.array([[1, 0, 0, 0, 0, 0, 0, 0]]).T
    )
    ...
    for frame in range(DH.rows) if symbolic else DH:
        ...
        fkDQ = (
            dqMultiplication(
                dqMultiplication(
                    dqMultiplication(
                        dqMultiplication(fkDQ, dqRz(DH[frame, 0], symbolic), symbolic),
                        dqTz(DH[frame, 1], symbolic),
                        symbolic,
                    ),
                    dqTx(DH[frame, 2], symbolic),
                    symbolic,
                ),
                dqRx(DH[frame, 3], symbolic),
                symbolic,
            )
            if symbolic
                ...
                dqMultiplication(
                    dqMultiplication(
                        dqMultiplication(fkDQ, dqRz(frame[0])), dqTz(frame[1])
                    ),
                    dqTx(frame[2]),
                ),
                dqRx(frame[3]),
    ...
    for i in range(robot.symbolicCOMs.shape[0]):
        ...
        COM = (
            dqMultiplication(
                dqMultiplication(
                    dqMultiplication(
                        dqRz(comDH[rowCOM, 0] if column >= 0 else 0, symbolic),
                        dqTz(comDH[rowCOM, 1] if column >= 1 else 0, symbolic),
                        symbolic,
                    ),
                    dqTx(comDH[rowCOM, 2] if column >= 2 else 0, symbolic),
                    symbolic,
                ),
                dqRx(comDH[rowCOM, 3] if column >= 3 else 0, symbolic),
                symbolic,
            )
            if symbolic
            else dqMultiplication(
                dqMultiplication(
                    dqMultiplication(
                        dqRz(comDH[rowCOM, 0] if column >= 0 else 0),
                        dqTz(comDH[rowCOM, 1] if column >= 1 else 0),
                    ),
                    dqTx(comDH[rowCOM, 2] if column >= 2 else 0),
                ),
                dqRx(comDH[rowCOM, 3] if column >= 3 else 0),
            )
        )

        # Forward kinematics to Center of Mass
        fkCOMDQ = (
            dqMultiplication(framesDQ[rowCOM - 1], COM, symbolic)
            if symbolic
            else dqMultiplication(framesDQ[rowCOM - 1], COM)
        )
...
def jacobianDQ(robot: object, symbolic=False):
    ...
    for j in range(n):
        ...
        J[:, j] = (
            ...
            * dqMultiplication(
                dqMultiplication(
                    dqMultiplication(fkDQ[row - 1], xi, symbolic),
                    conjugateDQ(fkDQ[row - 1]),
                    symbolic,
                ),
                fkDQ[-1],
                symbolic,
            ...
            * dqMultiplication(
                dqMultiplication(fkDQ[row - 1], xi), conjugateDQ(fkDQ[row - 1])
            ).flatten()
        )
...
def jacobianVelocityDQ(robot: object, symbolic=False):
    ...
    M = (
        Matrix([[eye(4), zeros(4)], [-crossOperatorExtension(r, symbolic), eye(4)]])
        if symbolic
        else np.append(
            np.append(np.eye(4), np.zeros((4, 4)), axis=1),
            np.append(-crossOperatorExtension(r), np.eye(4), axis=1),
            axis=0,
        )
    )

    return tidy(M * J) if symbolic else M.dot(J)
...
def jacobianVelocityDerivativeDQ(robot: object, symbolic=False):
    ...
    M_k = (
        Matrix([[zeros(4), zeros(4)], [-crossOperatorExtension(r, symbolic), eye(4)]])
        if symbolic
        else np.append(
            np.append(np.zeros((4, 4)), np.zeros((4, 4)), axis=1),
            np.append(-crossOperatorExtension(r), np.eye(4), axis=1),
            axis=0,
        )
    )
    ...
    for j in range(n):
        ...
        dAd = dqMultiplication(
            dqMultiplication(fkDQ[row - 1], xid, symbolic),
            conjugateDQ(fkDQ[row - 1], symbolic),
            symbolic,
        )
        ...
        for k in range(j):
            ...
            # Axis of actuation of current joint
            xi_k = (
                Matrix(robot.xi[:, k]) if symbolic else robot.xi[:, k].reshape((8, 1))
            )

            # Calculates the adjoint transformation of the current axis of actuation (xi_k)
            Ad_k = dqMultiplication(
                dqMultiplication(fkDQ[row_k - 1], xi_k, symbolic),
                conjugateDQ(fkDQ[row_k - 1], symbolic),
                symbolic,
            )

            # Non linear terms calculation for th Time Derivative of Jacobian Matrix
            dJ[:, j] += (
                dualCrossOperator(Ad_k * robot.qdSymbolic[k], symbolic) * Ad
                if symbolic
                else (
                    dualCrossOperator(Ad_k * robot.jointsVelocities[k]).dot(Ad)
                ).flatten()
            )
        ...
        dJ[:, j] = (
            M * (dAd + dJ[:, j])
            if symbolic
            else M.dot(dAd + dJ[:, j].reshape((8, 1))).flatten()
        )
        ...
        for k in range(n):
            ...
            # Non linear terms calculation for th Time Derivative of Jacobian Matrix
            dJ[:, j] += (
                dualCrossOperator(Ad, symbolic) * (M_k * Ad_k * robot.qdSymbolic[k])
                if symbolic
                else (
                    dualCrossOperator(Ad).dot(M_k.dot(Ad_k) * robot.jointsVelocities[k])
                ).flatten()
            )
