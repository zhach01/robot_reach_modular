...
def dqStateSpace(robot: object, symbolic=False):
    ...
    Xd = tidy(J * robot.qdSymbolic) if symbolic else J.dot(robot.jointsVelocities)
...
def dqDerivativeStateSpace(robot: object, symbolic=False):
    ...
    # Calculate state-space equation (symbolic or numerical)
    Xdd = (
        tidy((dJ * robot.qdSymbolic) + (J * robot.qddSymbolic))
        if symbolic
        else dJ.dot(robot.jointsVelocities) + J.dot(robot.jointsAccelerations)
    )
...
def dqStateSpaceCOM(robot: object, COM: int, symbolic=False):
    ...
    # Calculate state-space equation (symbolic or numerical)
    XdCOM = (
        tidy(JvdqCOM * robot.qdSymbolic)
        if symbolic
        else JvdqCOM.dot(robot.jointsVelocities)
    )
...
def dqDerivativeStateSpaceCOM(robot: object, COM: int, symbolic=False):
    ...
    # Calculate state-space equation (symbolic or numerical)
    XddCOM = (
        tidy((dJvdqCOM * robot.qdSymbolic) + (JvdqCOM * robot.qddSymbolic))
        if symbolic
        else dJvdqCOM.dot(robot.jointsVelocities)
        + JvdqCOM.dot(robot.jointsAccelerations)
    )
...
def dqVelocityPropagation(robot: object, w0: np.array, qd: np.array, symbolic=False):
    ...
    for k in range(1, m):
        ...
        if any(element == True for element in containedJoints):
            ...
            wJoint = (
                dqMultiplication(
                    dqMultiplication(Q, xi, symbolic),
                    conjugateDQ(Q, symbolic),
                    symbolic,
                )
                * qd[joint]
                if symbolic
                else dqMultiplication(dqMultiplication(Q, xi), conjugateDQ(Q))
                * qd[joint]
            )
        ...
        ri = crossOperatorExtension(
            dqToR3(fkDQ[k], symbolic) - dqToR3(fkDQ[k - 1], symbolic), symbolic
        )

        # Relative position matrix between i-th and i + 1 frames
        Mi = (
            Matrix([[eye(4), zeros(4)], [-ri, eye(4)]])
            if symbolic
            else np.append(
                np.append(np.eye(4), np.zeros((4, 4)), axis=1),
                np.append(-ri, np.eye(4), axis=1),
                axis=0,
            )
        )
        ...
        M = (
            Matrix([[eye(4), zeros(4)], [-r, eye(4)]])
            if symbolic
            else np.append(
                np.append(np.eye(4), np.zeros((4, 4)), axis=1),
                np.append(-r, np.eye(4), axis=1),
                axis=0,
            )
        )
        ...
        w = (
            tidy((Mi * W[-1]) + (M * wJoint))
            if symbolic
            else Mi.dot(W[-1]) + M.dot(wJoint)
        )

        # Append each calculated velocity
        W.append(w if symbolic else w)

    return W


def dqAccelerationPropagation(
    robot: object, dw0: np.array, Wdq: list, qd: np.array, qdd: np.array, symbolic=False
):
    ...
    for k in range(1, m):
        ...
        if any(element == True for element in containedJoints):
            ...
            dwJoint = (
                dqMultiplication(
                    dqMultiplication(
                        Q, (xid * qd[joint]) + (xi * qdd[joint]), symbolic
                    ),
                    conjugateDQ(Q, symbolic),
                    symbolic,
                )
                if symbolic
                else dqMultiplication(
                    dqMultiplication(Q, (xid * qd[joint]) + (xi * qdd[joint])),
                    conjugateDQ(Q),
                )
            )
            ...
            dualW = (
                dqMultiplication(
                    dqMultiplication(
                        conjugateDQ(Q, symbolic), Mi * Wdq[k - 1], symbolic
                    ),
                    Q,
                    symbolic,
                )
                if symbolic
                    ...
                    dqMultiplication(conjugateDQ(Q), Mi.dot(Wdq[k - 1])), Q
                )
            )
            ...
            dwCentripetalJoint = (
                dqMultiplication(
                    dqMultiplication(
                        Q, dualCrossOperator(dualW, symbolic) * xi * qd[joint], symbolic
                    ),
                    conjugateDQ(Q, symbolic),
                    symbolic,
                )
                if symbolic
                    ...
                    dqMultiplication(Q, dualCrossOperator(dualW).dot(xi * qd[joint])),
                    conjugateDQ(Q),
                )
            )
        ...
        dwCentripetal = (
            Matrix(
                [
                    ...
                    [
                        (
                            crossOperatorExtension(Wdq[k][0:4, :], symbolic)
                            * Wdq[k][4:8, :]
                        )
                        - (
                            crossOperatorExtension(Wdq[k - 1][0:4, :], symbolic)
                            * Wdq[k - 1][4:8, :]
                        )
                    ],
                ]
            ...
            if symbolic
                ...
                np.zeros((4, 1)),
                crossOperatorExtension(Wdq[k][0:4, :]).dot(Wdq[k][4:8, :])
                - crossOperatorExtension(Wdq[k - 1][0:4, :]).dot(Wdq[k - 1][4:8, :]),
                axis=0,
            )
        )
        ...
        dw = (
            tidy((Mi * dW[-1]) + dwCentripetal + M * (dwCentripetalJoint + dwJoint))
            if symbolic
            else Mi.dot(dW[-1]) + dwCentripetal + M.dot(dwCentripetalJoint + dwJoint)
        )

        # Append each calculated angular velocity
        dW.append(dw if symbolic else dw)

    return dW


def dqVelocityPropagationCOM(
    robot: object, WdqCOM0: np.array, Wdq: list, qd: np.array, symbolic=False
):
    ...
    for k in range(1, m):
        ...
        if any(element == True for element in containedCOMs):
            ...
            ri = crossOperatorExtension(
                dqToR3(fkCOMDQ[COM], symbolic) - dqToR3(fkDQ[k - 1], symbolic), symbolic
            )
            ...
            Mcom = (
                Matrix([[eye(4), zeros(4)], [-rCOM, eye(4)]])
                if symbolic
                else np.append(
                    np.append(np.eye(4), np.zeros((4, 4)), axis=1),
                    np.append(-rCOM, np.eye(4), axis=1),
                    axis=0,
                )
            )
            ...
            # Calculate velocity up to this point
            wCOM = (
                tidy((Mi * Wdq[k - 1]) + (Mcom * wJoint))
                if symbolic
                else Mi.dot(Wdq[k - 1]) + Mcom.dot(wJoint)
            )

            # Append each calculated velocity
            WdqCOM.append(wCOM if symbolic else wCOM)

    return WdqCOM


def dqAccelerationPropagationCOM(
    robot: object,
    dWdqCOM0: np.array,
    Wdq: list,
    WdqCOM: list,
    ...
    for k in range(1, m):
        ...
        if any(element == True for element in containedJoints):
            ...
            # Create position matrix for i-th reference frame
            Mi = (
                Matrix([[eye(4), zeros(4)]])
                if symbolic
                else np.append(np.eye(4), np.zeros((4, 4)), axis=1)
            )  # placeholder row; replaced below to keep structure identical
            Mi = (
                Matrix([[eye(4), zeros(4)], [ri, eye(4)]])
                if symbolic
                else np.append(Mi, np.append(ri, np.eye(4), axis=1), axis=0)
            )
        ...
        if any(element == True for element in containedCOMs):
            ...
            dwCentripetalCOM = (
                Matrix(
                    [
                        ...
                        [
                            (
                                crossOperatorExtension(WdqCOM[COM][0:4, :], symbolic)
                                * WdqCOM[COM][4:8, :]
                            )
                            - (
                                crossOperatorExtension(Wdq[k - 1][0:4, :], symbolic)
                ...
                if symbolic
                    ...
                    np.zeros((4, 1)),
                    crossOperatorExtension(WdqCOM[COM][0:4, :]).dot(WdqCOM[COM][4:8, :])
                    - crossOperatorExtension(Wdq[k - 1][0:4, :]).dot(
                        Wdq[k - 1][4:8, :]
                    ),
                    axis=0,
                )
            ...
            dwCOM = (
                tidy(
                    (Mi * dWdq[k - 1])
                    + dwCentripetalCOM
                    + Mcom * (dwCentripetalJoint + dwJoint)
                )
                if symbolic
                else Mi.dot(dWdq[k - 1])
                + dwCentripetalCOM
                + Mcom.dot(dwCentripetalJoint + dwJoint)
            )

            # Append each calculated velocity
            dWdqCOM.append(dwCOM if symbolic else dwCOM)
