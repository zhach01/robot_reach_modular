...
for tk in D:
    ...
    for j in range(min(len(st),len(en))):
        ...
        tm=np.linspace(0,1,act.shape[0]);ea,eb=a*EF//KF,b*EF//KF;te=np.linspace(0,1,eb-ea)
        for col,row,lab in PAIRS:
            mo=np.interp(ph,tm,act[:,col]);mo/=(mo.max() or 1);me=np.interp(ph,te,envf(E[row][ea:eb]));me/=(me.max() or 1)
            M[lab].append(mo);Eacc[lab].append(me)
print("=== mean-profile correlation (averaged pattern), HS01, %d reaches ==="%len(M[PAIRS[0][2]]))
fig,ax=plt.subplots(1,4,figsize=(14,3.2))
for i,(c,r,lab) in enumerate(PAIRS):
    Mm=np.array(M[lab]).mean(0);Em=np.array(Eacc[lab]).mean(0);rr=np.corrcoef(Mm,Em)[0,1]
    print(f"  {lab:22s} mean-profile r = {rr:+.2f}")
    ax[i].plot(ph*100,Em,"k",lw=2,label="EMG");ax[i].plot(ph*100,Mm,"r--",lw=2,label="Model")
    ax[i].set_title(f"{lab}\nr={rr:+.2f}",fontsize=9);ax[i].set_xlabel("reach %");ax[i].grid(ls="--",alpha=.3);ax[i].set_ylim(-0.05,1.1)
ax[0].legend(fontsize=8);ax[0].set_ylabel("normalised (mean)")
fig.suptitle("Mean activation pattern: model vs EMG (HS01). Agonists reproduced; antagonist co-contraction underestimated.",fontsize=10)
fig.tight_layout();fig.savefig(f"{B}/kinarm_dataset/emg_meanprofile.png",dpi=110,bbox_inches="tight");print("wrote emg_meanprofile.png")
