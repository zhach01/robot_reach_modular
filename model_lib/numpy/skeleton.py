...
Uses:
- lib.numpy.Robot.Serial for the model container
- lib.kinematics.numpy.HTM_kinematics.{forwardHTM, geometricJacobian}
- lib.dynamics.numpy.DynamicsHTM.{inertiaMatrixCOM, centrifugalCoriolisCOM, gravitationalCOM}
...
from lib.numpy.Robot import Serial
import lib.kinematics.numpy.HTM_kinematics as _htm
import lib.dynamics.numpy.DynamicsHTM as _dyn
...
def geometricJacobianDot_cached(robot, symbolic=False):
    """
    Cached wrapper around lib.kinematics.numpy.HTM_kinematics.geometricJacobianDerivative(robot).
    ...
    * If the library doesn't expose the derivative, we raise a clear error.
    ...
    if not hasattr(_htm, "geometricJacobianDerivative"):
        raise AttributeError(
            "lib.kinematics.numpy.HTM_kinematics.geometricJacobianDerivative(...) not found. "
...
class TwoDofArm(Skeleton):
    ...
    where:
        - D, C, g come from lib.dynamics.numpy.DynamicsHTM
        - J is lib.kinematics.numpy.HTM_kinematics.geometricJacobian (linear x-y rows)
    """
