...
# shoulder flexor: model pec col0 vs EMG ant deltoid row2 ; shoulder ext: model delt col1 vs post deltoid row0
MF=[];EFp=[];ME=[];EEp=[]
for tk in D:
    ...
    for j in range(min(len(st),len(en))):
        ...
        mf=np.interp(ph,tm,act[:,0]);mf/= (mf.max() or 1); ef=np.interp(ph,te,envf(E[2][ea:eb]));ef/=(ef.max() or 1)
        me=np.interp(ph,tm,act[:,1]);me/=(me.max() or 1); ee=np.interp(ph,te,envf(E[0][ea:eb]));ee/=(ee.max() or 1)
        MF.append(mf);EFp.append(ef);ME.append(me);EEp.append(ee)
MF,EFp,ME,EEp=map(np.array,(MF,EFp,ME,EEp))
def pr(name,mod,emg):
    perreach=np.nanmean([np.corrcoef(mod[i],emg[i])[0,1] for i in range(len(mod))])
    meanprof=np.corrcoef(mod.mean(0),emg.mean(0))[0,1]
    print(f"  {name:20s} per-reach r = {perreach:+.2f}   |   MEAN-PROFILE r = {meanprof:+.2f}")
print("reaches",len(MF))
pr("Shoulder flexor",MF,EFp); pr("Shoulder extensor",ME,EEp)
BI=[];BE=[];TI=[];TE=[]
